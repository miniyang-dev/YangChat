...
TOOLS = [
    {
        ...
        "function": {
            "name": "web_search",
            "description": "搜尋網路上的最新資訊，取得真實、即時的數據後再回答。",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
...
# 固定用最快最便宜的模型做分類，不跟著使用者選的主模型跑
_CLASSIFIER_MODEL = "claude-haiku-4-5"

_CLASSIFIER_PROMPT = """\
判斷以下問題是否需要搜尋網路才能正確回答。

【需要搜尋】的情況（回答 yes）：
- 股票、指數、ETF 的股價、漲跌、走勢、技術分析、籌碼、法人動向
- 天氣、溫度、降雨、空氣品質
- 外匯、加密貨幣、商品期貨價格
- 今天/昨天/最近/最新的新聞、事件、賽事結果
- 特定公司最新財報、公告、人事異動
- 任何需要「今天日期」才能正確回答的問題
...
只回答 yes 或 no，不要任何其他文字。

問題：{question}"""

async def _needs_search(messages: list) -> bool:
    """
    Two-pass 分類器：用輕量模型判斷最後一則 user 訊息是否需要即時搜尋。
    失敗時 fallback 到 True（寧可多搜也不要漏搜）。
    """
    # 取最後一則 user 訊息
    last_user = ""
    ...
    try:
        resp = await client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": _CLASSIFIER_PROMPT.format(question=last_user),
            }],
            max_tokens=5,
        )
...
async def chat_complete(messages: list, model: str) -> str:
    ...
    tool_choice = "required" if await _needs_search(messages) else "none"

    # tool_choice="none" 時不帶 tools，節省 token
    if tool_choice == "none":
        response = await client.chat.completions.create(
            model=model,
            messages=msgs,
...
async def chat_stream(
    ...
    流程：
      0. Two-pass 分類器判斷是否需要搜尋
      1. 需要搜尋 → tool_choice="required"，第二輪起改 "auto"
         不需要搜尋 → 直接 stream，完全不帶 tools（省 token）
      2. 收集 delta；有 tool_call chunk 就組裝後執行，重新 stream
      3. 純文字 → 直接 yield SSE chunks
    """
    client = get_client()
    msgs = _inject_default_system(list(messages))
    tool_choice = "required" if await _needs_search(messages) else "none"

    # 不需要搜尋 → 直接 streaming，不掛 tools
    if tool_choice == "none":
    ...
    for round_num in range(5):  # 最多 5 輪 tool call
        stream = await client.chat.completions.create(
            model=model,
            messages=msgs,
            tools=TOOLS,  # type: ignore[arg-type]
