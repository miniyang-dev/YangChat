...
- **即時資料**：涉及股票/指數/股價/技術分析/天氣/匯率等問題，必須先呼叫 web_search 取得最新數據，不得使用訓練資料中的舊資料作答
...
TOOLS = [
    {
        ...
        "function": {
            "name": "web_search",
            "description": (
                "搜尋網路上的最新資訊。以下情況【必須】呼叫此工具，不可用記憶回答："
                "① 任何股票、指數、ETF 的股價、漲跌、成交量、技術指標、籌碼分析（即使問的是『分析』或『走勢』）；"
                "② 天氣、降雨、溫度；"
                "③ 外匯、加密貨幣、商品期貨價格；"
                "④ 最新新聞、賽事、即時事件；"
                "⑤ 訓練資料截止後的任何事實。"
                "切勿用訓練資料中的舊資料回答金融市場問題，必須先搜尋取得最新數據再作答。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
